...
public interface OrderApiCtrl {
    ...
    ResponseEntity<ReadableOrderConfirmation> checkout(@PathVariable String code, @Valid @RequestBody PersistableOrder order);

    @PostMapping("/api/v1/cart/{code}/checkout")
    ResponseEntity<ReadableOrderConfirmation> checkout(@PathVariable String code, @Valid @RequestBody PersistableAnonymousOrder order);
    ...
    ResponseEntity<ReadableOrder> getOrder(@PathVariable Long id);
    ...
    ResponseEntity<ReadableOrder> get(@PathVariable Long id);

    @PatchMapping("/api/v1/private/orders/{id}/customer")
    ResponseEntity<Void> updateOrderCustomer(@PathVariable Long id, @Valid @RequestBody Customer orderCustomer);

    @PutMapping("/api/v1/private/orders/{id}/status")
    ResponseEntity<Void> updateOrderStatus(@PathVariable Long id, @Valid @RequestBody String status);
}